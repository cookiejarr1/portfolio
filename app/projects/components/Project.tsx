...
export default function Project({ project, image }: { project: Project; image?: StaticImageData }) {
  ...
  return (
    <Card
      shadow="sm"
      className="group border border-default-200 dark:border-white/5 bg-white/95 dark:bg-[#121418] rounded-[22px] transition-all duration-300 hover:shadow-2xl hover:-translate-y-1"
    >
      <Link href={`/projects/${project.id}`} className="block h-full">
        <div className="relative w-full h-[260px] overflow-hidden rounded-t-[22px] bg-gradient-to-b from-[#585C61]/40 to-transparent">
          <NextImage
            src={image || project.image || "/placeholder.png"}
            alt={project.title}
            fill
            className="object-cover object-top cursor-pointer transition-transform duration-500 ease-out group-hover:scale-110"
        ...
        <CardBody className="px-6 py-5 flex flex-col gap-4">
          <div className="flex items-start justify-between gap-3">
            <div className="flex flex-wrap gap-2">
              ...
              {project.tags.length > tagLength && (
                <Tooltip showArrow={true} content={hiddenTags.join(", ")}>
                  <Chip
                    size="sm"
                    variant="flat"
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs leading-tight text-foreground/70">{project.description}</p>
          </div>
          <div className="flex items-center justify-between pt-2 text-sm font-medium">
            ...
            <span className="inline-flex items-center gap-1 text-sky-500 transition-all duration-300 group-hover:gap-2 hover:underline underline-offset-2">
              Read more
